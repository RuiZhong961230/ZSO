...
def ZSO(func):
    ...
    alpha = 2 * (1 - curIter / MaxIter)
    beta = 2 * (1 - curIter / MaxIter)
    for i in range(PopSize):
...
def RunZSO(func):
    ...
    np.savetxt("./ZSO_Linear_Data/CEC2014/F" + str(FuncNum) + "_" + str(DimSize) + "D.csv", All_Trial_Best, delimiter=",")
...
def main(dim):
    ...
    FuncNum = 0
    for i in range(21, len(CEC2014Funcs)):
        FuncNum = i + 1
        RunZSO(CEC2014Funcs[i])


if __name__ == "__main__":
    if os.path.exists('./ZSO_Linear_Data/CEC2014') == False:
        os.makedirs('./ZSO_Linear_Data/CEC2014')
    Dims = [50]
    for Dim in Dims:
        main(Dim)
